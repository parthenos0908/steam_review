...
def loadForums(url, label, page_MAX):
    ...
    for i in tqdm(range(page_MAX)):
        ...
        html = urllib.request.urlopen(
            factorio_url + url + "&start=" + str(i*25))
        ...
        for i, thread in enumerate(tqdm(threads, leave=False)):
            ...
            if thread_url:
                ...
                forum_url = factorio_url + thread_url[2:]
...
def loadThread(url):
    try:
        ...
        soup = BeautifulSoup(html, "html.parser")
        comment = soup.find_all(class_="content").get_text()
        print(comment)
        return comment
...
if __name__ == '__main__':
    ...
    loadThread("https://forum.scssoft.com/viewtopic.php?f=5&t=293432#p1474657")
    # main()
