...
def main():
    ...
    # labelをintに変換
    label_list = to_int_label(label_list)

    # テキストとラベルの関係を維持したままリストをシャッフル
    p = list(zip(text_list, label_list))
    random.shuffle(p)
    text_list, label_list = zip(*p)

    # 訓練データ
    train_texts = text_list[:int(len(text_list)*0.7)]
    train_labels = label_list[:int(len(label_list)*0.7)]

    # テストデータ
    test_texts = text_list[int(len(text_list)*0.7):]
    test_labels = label_list[int(len(label_list)*0.7):]
...
if __name__ == '__main__':
    # model = build_model(model_name, num_classes=num_classes,
    #                     max_length=max_length)
    # model_plot_name = path.join(path.dirname(__file__), "model.png")
    # plot_model(model, to_file=model_plot_name, show_shapes=True, expand_nested=True)
    main()
